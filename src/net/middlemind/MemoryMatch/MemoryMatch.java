...
public final class MemoryMatch {
     ...
     */
    public static String ENGINE_CONFIG_FILE = "../cfg/engine_config_mmg_memory_match_pro.xml";
    ...
    public static final void main(String[] args) {
        ...
        try {
            if (MemoryMatch.ENGINE_CONFIG_FILE != null && MemoryMatch.ENGINE_CONFIG_FILE.equals("") == false) {
                GameSettingsImporter dci = new GameSettingsImporter();
                boolean r = dci.ImportGameSettings(MemoryMatch.ENGINE_CONFIG_FILE);
                    ...
                    for (int i = 0; i < len; i++) {
                        try {
                            ...
                            if (ent.from != null && ent.from.equals("GameSettings") == true) {
                                ...
                                f = MemoryMatch.class.getField(ent.key);
                                if (f != null) {
                                    System.out.println("Importing " + ent.from + " field: " + ent.key + " with value: " + ent.val + " with type: " + ent.type + " from: " + ent.from);
                                    SetField(ent, f);
                                }             
                            ...
                            if(ent != null) {
                                ...
                                System.out.println("Ignoring dat constants field: ent is NULL");
                            }
                            MmgHelper.wrErr(e);
                        }
                    }
        ...
        mf = new MainFrame(MemoryMatch.WIN_WIDTH, MemoryMatch.WIN_HEIGHT, MemoryMatch.PANEL_WIDTH, MemoryMatch.PANEL_HEIGHT, MemoryMatch.GAME_WIDTH, MemoryMatch.GAME_HEIGHT);
        pnlGame = new GamePanel(mf, MemoryMatch.PANEL_WIDTH, MemoryMatch.PANEL_HEIGHT, (MemoryMatch.WIN_WIDTH - MemoryMatch.PANEL_WIDTH) / 2, (MemoryMatch.WIN_HEIGHT - MemoryMatch.PANEL_HEIGHT) / 2, MemoryMatch.GAME_WIDTH, MemoryMatch.GAME_HEIGHT);
        mf.SetGamePanel(pnlGame);
        mf.InitComponents();
        fr = new RunFrameRate(mf, FPS);

        mf.setSize(MemoryMatch.WIN_WIDTH, MemoryMatch.WIN_HEIGHT);
